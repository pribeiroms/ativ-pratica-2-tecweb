...
function validar(form) {
  ...
  if (!form.nome.trim()) {
    erros.nome = 'O nome do item e obrigatorio.'
  }
  if (!form.categoria) {
    erros.categoria = 'Selecione uma categoria.'
  }
  if (!form.preco) {
    erros.preco = 'O preco estimado e obrigatorio.'
  } else if (Number(form.preco) <= 0) {
    erros.preco = 'O preco deve ser maior que zero.'
  }
  if (!form.prioridade) {
...
function Cadastro() {
  ...
  return (
    <section className="page cadastro-page">
      ...
      <h1>Novo presente</h1>
      <p className="lead">
        Preencha os dados do item que deseja adicionar a sua wishlist.
      </p>
      ...
      <form className="cadastro-form" onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-group">
          <label htmlFor="preco">Preco estimado (R$)</label>
          <input
            id="preco"
            name="preco"
            type="number"
